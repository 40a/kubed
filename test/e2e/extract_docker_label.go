	...
	Describe("Adding Annotaions in StatefulSet", func() {
		...
		FContext("When docker image contains labels", func() {
			BeforeEach(func() {
				...
				statefulset = f.NewStatefulSet(name, namespace, labels, containers1, svc.Name)
			})
		...
		Context("When docker image doesn't contain any labels", func() {
			BeforeEach(func() {
				service = f.NewService(name, namespace, labels)
				statefulset = f.NewStatefulSet(name, namespace, labels, containers2, svc.Name)
			})
		...
		Context("When docker images aren't found with the provided secrets", func() {
			BeforeEach(func() {
				service = f.NewService(name, namespace, labels)
				statefulset = f.NewStatefulSet(name, namespace, labels, containers3, svc.Name)
			})
